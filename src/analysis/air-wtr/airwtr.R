...
# dv longterm data
dv <- read_feather('data/munged/dv/combined/dv_longterm_air_wtr.feather')
...
site_info <- read.csv('data/munged/sites/sites_dv_filtered_compiled.csv', colClasses = 'character')
site_info <- site_info %>%
  select(
    site_code,
    lat,
...
scatter_simple(dv.plot, 'air.tmean', 'wtr.tmean', 'year', stat = "Mean", discrete = TRUE) +
